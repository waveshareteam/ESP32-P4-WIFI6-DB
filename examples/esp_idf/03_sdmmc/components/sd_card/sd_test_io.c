...
static void print_pin_voltage(float voltage)
{
    if (isnan(voltage)) {
        printf(" N/A   ");
    } else {
...
void check_sd_card_pins(pin_configuration_t *config, const int pin_count)
{
    ...
    if (adc_cali_handle == NULL || do_calibration == NULL) {
        ESP_LOGE(TAG, "Allocate ADC calibration state failed");
    ...
    for (int i = 0; i < pin_count; i++) {
        do_calibration[i] = adc_calibration_init(
            CONFIG_EXAMPLE_ADC_UNIT, config->adc_channels[i], ADC_ATTEN_DB, &adc_cali_handle[i]
        );
    }
    ...
    for (int i = 0; i < pin_count; ++i) {
        ...
        for (int j = 0; j < pin_count; ++j) {
            ...
            usleep(100);
            float voltage = get_pin_voltage(config->adc_channels[j], adc_handle,
                                            do_calibration[j], adc_cali_handle[j]);
            print_pin_voltage(voltage);
        }
        printf("\n");
