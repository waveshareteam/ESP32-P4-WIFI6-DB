...
/**
 * @brief Project-local MIPI-DSI LCD driver compilation
 *
 * Select exactly one project-local LCD driver for this example. These options
 * are separate from the library's built-in LCD driver options above.
 */
#define ESP_PANEL_DRIVERS_LCD_ENABLE_HX8394_LOCAL       (0)
#define ESP_PANEL_DRIVERS_LCD_ENABLE_ILI9881C_LOCAL     (0)
#define ESP_PANEL_DRIVERS_LCD_ENABLE_JD9365_LOCAL       (1)

#if (ESP_PANEL_DRIVERS_LCD_ENABLE_HX8394_LOCAL + \
     ESP_PANEL_DRIVERS_LCD_ENABLE_ILI9881C_LOCAL + \
     ESP_PANEL_DRIVERS_LCD_ENABLE_JD9365_LOCAL) != 1
    #error "Select exactly one project-local MIPI-DSI LCD driver"
#endif

/**
 * @brief Project-local JD9365 panel profile
 *
...
#define CONFIG_BSP_LCD_TYPE_720_720_4_INCH               (0)
         ...
         CONFIG_BSP_LCD_TYPE_720_720_4_INCH) != 1
        #error "Select exactly one project-local JD9365 panel profile"
    #endif
#endif
