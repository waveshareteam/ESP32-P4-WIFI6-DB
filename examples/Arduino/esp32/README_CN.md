...
本目录提供 `ESP32-P4-WIFI6-DB` 在 Arduino IDE 中的板卡注册和 variant 文件，
...
- 板卡条目：`Waveshare ESP32-P4-WIFI6-DB`
- 编译目标：`esp32p4`
- Flash：`32MB`
...
2. 若安装目录已经包含其他板卡或本地修改，只合并以
   `waveshare_esp32_p4_wifi6_db.` 开头的板卡条目，避免覆盖其他定义。
3. 将 `variants/waveshare_esp32_p4_wifi6_db` 整个目录复制到同版本 Arduino-ESP32
   的 `variants` 目录。
4. 重启 Arduino IDE，选择 `Waveshare ESP32-P4-WIFI6-DB`。
...
`pins_arduino.h` 按 `firmware/esp32_p4_wifi6_db` BSP 和该工程原理图适配：
...
当前 BSP 和两个 Arduino 显示示例默认使用 10.1 英寸 A 型 JD9365（800×1280）；
...
`mipi_dsi` 和 `mipi_csi` 仍需按实际 LCD 面板选择对应的控制器参数；其中
`mipi_csi` 当前默认是 10.1 英寸 A 型 JD9365，不应继续使用 7C 的
EK79007/1024×600/GPIO30/31 配置。

I2C 初始化方式也按示例区分：`mipi_dsi` 使用 legacy I2C API，`mipi_csi` 使用
`Wire1` 建立新版总线后共享 `i2c_master` 句柄；不要把一个示例的 I2C
初始化代码复制到另一个示例中。
