...
static constexpr lv_coord_t PAGE_TOP_OFFSET = 60;
...
inline lv_obj_t *create_page(lv_obj_t *screen)
{
    ...
    lv_obj_set_height(page, lv_disp_get_ver_res(nullptr) - PAGE_TOP_OFFSET);
    lv_obj_align(page, LV_ALIGN_TOP_MID, 0, PAGE_TOP_OFFSET);
    ...
    lv_obj_t *page, const char *title, lv_event_cb_t back_cb = nullptr, void *user_data = nullptr)
{
    ...
    if (back_cb != nullptr) {
        ...
        lv_obj_align(back_button, LV_ALIGN_LEFT_MID, 20, 0);
    ...
    const lv_coord_t title_x = (back_button != nullptr) ? 84 : PAGE_HORIZONTAL_MARGIN;
    lv_coord_t title_width = lv_obj_get_width(page) - title_x - PAGE_HORIZONTAL_MARGIN;
    if (title_width < 1) {
        title_width = 1;
    }
...
{
    lv_obj_update_layout(page);
    lv_coord_t content_width = lv_obj_get_width(page) - PAGE_HORIZONTAL_MARGIN * 2;
    lv_coord_t content_height = lv_obj_get_height(page) - PAGE_HEADER_HEIGHT -
                                CONTENT_TOP_GAP - CONTENT_BOTTOM_MARGIN;
    if (content_width < 1) {
        content_width = 1;
    }
    if (content_height < 1) {
    ...
    lv_obj_align(list, LV_ALIGN_TOP_MID, 0, PAGE_HEADER_HEIGHT + CONTENT_TOP_GAP);
