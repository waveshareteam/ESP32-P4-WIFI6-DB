...
namespace esp_brookesia::apps
{
    ...
    private:
        ...
        // The board BSP uses the ES8311 with one analog microphone. Capture is
        // configured as mono, so each I2S frame contains one microphone sample.
        ...
        static constexpr uint32_t AUDIO_TASK_STOP_TIMEOUT_MS = 2000;

        lv_obj_t *_canvas;
        lv_obj_t *_mic_label;
        lv_timer_t *_timer;
        SemaphoreHandle_t _audio_task_done_sem;
        std::atomic<bool> _audio_task_running;
        std::atomic<bool> _audio_task_join_pending;
        std::atomic<bool> _capture_failed;
        std::atomic<bool> _audio_session_acquired{false};
        std::mutex _spectrum_mutex;
        ...
        bool activateAudio();
        bool suspendAudio();
        static bool focus_activate(void *user_data);
        static bool focus_suspend(void *user_data);
