...
constexpr uint32_t SAMPLE_RATE = BSP_EXTRA_XIAOZHI_SAMPLE_RATE;
...
{
    cancel();
    return waitUntilIdle(timeout_ms);
}
...
void XiaozhiActivationPrompt::run()
{
    ...
    const bsp_extra_audio_session_config_t audio_config = {
        .enable_input = false,
        .enable_output = true,
        .sample_rate = SAMPLE_RATE,
        .bits_per_sample = BSP_EXTRA_XIAOZHI_BITS,
        .channel_mode = I2S_SLOT_MODE_STEREO,
    };
    ...
    focus_requested = audio_focus::manager::instance().request(
                          audio_focus::client::prompt, {}
                      );
    focus_acquired = focus_requested &&
                     audio_focus::manager::instance().is_active(
                         audio_focus::client::prompt
                     );
    if (focus_acquired) {
        codec_result = bsp_extra_audio_acquire(
                           BSP_EXTRA_AUDIO_CLIENT_PROMPT,
                           &audio_config,
                           pdMS_TO_TICKS(1000)
                       );
    }
    session_acquired = codec_result == ESP_OK;
    if (!shouldContinue()) {
        goto cleanup;
    }
...
cleanup:
    ...
    if (session_acquired) {
        esp_err_t release_ret = bsp_extra_audio_release(
                                    BSP_EXTRA_AUDIO_CLIENT_PROMPT,
                                    pdMS_TO_TICKS(1000)
                                );
        if (release_ret != ESP_OK) {
            ESP_UTILS_LOGE("Release activation prompt audio failed: %s",
                           esp_err_to_name(release_ret));
        }
    }
    if (focus_requested) {
        (void)audio_focus::manager::instance().abandon(audio_focus::client::prompt);
    }
