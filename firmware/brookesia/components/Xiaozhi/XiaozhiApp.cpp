...
#include "board/bsp_extra_xiaozhi.h"
...
constexpr uint32_t DEVICE_AUDIO_SAMPLE_RATE = BSP_EXTRA_XIAOZHI_SAMPLE_RATE;
...
constexpr size_t PLAYBACK_RING_BUFFER_BYTES = 128 * 1024;
...
public:
    ...
    ~ScopedXiaozhiInfo()
    {
        xiaozhi_services_free_info(&value);
    }

    xiaozhi_services_info_t value = {};
...
bool XiaozhiApp::init()
{
    ...
    if (!bsp_extra_wlan_wifi_init()) {
        setError(ESP_FAIL, "Initialize board Wi-Fi service");
        return false;
    }
...
bool XiaozhiApp::deinit()
{
    ...
    if (_playback_queue) {
        vRingbufferDeleteWithCaps(_playback_queue);
...
bool XiaozhiApp::prepareChat()
{
    ...
    esp_err_t ret = xiaozhi_services_get_info(&service.value);
    if (!canPrepare()) {
        return false;
    }
    if (ret != ESP_OK) {
    ...
    callbacks.onAudio = [this, client_generation](
        ...
        if (len > sizeof(AudioPacket::data)) {
            ESP_UTILS_LOGW(
                "Drop Xiaozhi audio packet: %u bytes exceeds %u-byte buffer",
                static_cast<unsigned int>(len),
                static_cast<unsigned int>(sizeof(AudioPacket::data))
            );
            return;
        }

        State state = _state.load();
        const bool playback_window =
            state == State::Listening || state == State::Processing ||
            state == State::Speaking;
        if (!_accept_playback.load() && !playback_window) {
            ESP_UTILS_LOGW(
                "Drop Xiaozhi audio packet before playback window (state=%u)",
                static_cast<unsigned int>(state)
            );
            return;
        }
        if (playback_window && !_accept_playback.exchange(true)) {
            ESP_UTILS_LOGI("Accept Xiaozhi audio before TTS_START event");
        }
        ...
        if (_accept_playback.load() &&
                ...
                _channel_state.load() == ChannelState::Open) {
            if (xRingbufferSend(
                    _playback_queue, &packet, sizeof(packet), 0
                ) == pdTRUE) {
                _last_audio_packet_tick.store(xTaskGetTickCount());
            } else {
                ESP_UTILS_LOGW(
                    "Drop Xiaozhi audio packet: playback ring buffer is full"
                );
            }
        }
    };
    ...
    XiaozhiClient::Config protocol_config(
        ...
        use_websocket,
        CHAT_SAMPLE_RATE,
        static_cast<int>(AFE_INPUT_CHANNELS),
...
{
    const bool input_resamplers_ready = _input_resamplers[0] != nullptr;
    ...
    if (!audio_focus::manager::instance().request(
            audio_focus::client::xiaozhi, {})) {
        setError(ESP_ERR_INVALID_STATE, "Acquire Xiaozhi audio focus");
        return false;
    }

    esp_err_t ret = bsp_extra_xiaozhi_audio_start();
    if (ret != ESP_OK) {
        (void)audio_focus::manager::instance().abandon(audio_focus::client::xiaozhi);
...
void XiaozhiApp::shutdownAudio()
{
    ...
    if (_playback_mutex &&
        ...
        if (_playback_queue) {
            (void)vRingbufferReset(_playback_queue);
        }
    ...
    (void)bsp_extra_xiaozhi_audio_stop();
    (void)audio_focus::manager::instance().abandon(audio_focus::client::xiaozhi);
}
...
)
{
    ...
    ESP_UTILS_LOGI(
        ...
        static_cast<unsigned int>(sample_rate),
        static_cast<unsigned int>(frame_duration_ms),
        sample_rate == DEVICE_AUDIO_SAMPLE_RATE ? "" : ", resampled to board rate"
    );
    return true;
}
...
bool XiaozhiApp::playbackDrainReady() const
{
    ...
    vRingbufferGetInfo(
        _playback_queue, nullptr, nullptr, nullptr, nullptr, &items_waiting
    );
    if (items_waiting != 0) {
        return false;
    }
...
{
    if (!waitForPlaybackIdle(PLAYBACK_IDLE_WAIT_MS) ||
            !_playback_mutex ||
            xSemaphoreTake(
    ...
    bool ok = true;
    if (_playback_queue) {
        esp_err_t reset_ret = vRingbufferReset(_playback_queue);
        if (reset_ret != ESP_OK) {
            ESP_UTILS_LOGW(
                "Reset Xiaozhi playback ring buffer failed: %s",
...
void XiaozhiApp::inputAudio()
{
    const size_t input_bytes =
        INPUT_READ_FRAMES * BSP_EXTRA_XIAOZHI_CHANNELS * sizeof(int16_t);
    auto *device_pcm = static_cast<int16_t *>(malloc(input_bytes));
    auto *chat_pcm = static_cast<int16_t *>(
        malloc(_input_resampler_max_samples * AFE_INPUT_CHANNELS * sizeof(int16_t))
    );
    uint32_t read_failures = 0;
    uint32_t resample_failures = 0;
    const bool input_resampler_ready = _input_resamplers[0] != nullptr;
    if (!device_pcm || !chat_pcm || !input_resampler_ready) {
        free(device_pcm);
        free(chat_pcm);
        ControlEvent event = {};
    ...
    while (_audio_tasks_running.load()) {
        ...
        size_t bytes_read = 0;
        esp_err_t ret = bsp_extra_xiaozhi_audio_read(
            device_pcm, input_bytes, &bytes_read, 100
        );
        if (ret == ESP_OK && bytes_read > 0 && _audio_processor) {
            read_failures = 0;
            const uint32_t input_frames = static_cast<uint32_t>(
                bytes_read / (BSP_EXTRA_XIAOZHI_CHANNELS * sizeof(int16_t))
            );
            uint32_t peak = 0;
            uint64_t absolute_sum = 0;
            for (uint32_t frame = 0; frame < input_frames; ++frame) {
                const int16_t sample = device_pcm[frame];
            ...
            _input_pcm_mean_abs.store(
                input_frames ?
                    static_cast<uint32_t>(absolute_sum / input_frames) : 0
            );

            uint32_t output_samples = _input_resampler_max_samples;
            int resample_ret = esp_ae_rate_cvt_process(
                _input_resamplers[0],
                reinterpret_cast<esp_ae_sample_t>(device_pcm),
                input_frames,
                reinterpret_cast<esp_ae_sample_t>(chat_pcm),
                &output_samples
            );
            if (resample_ret == ESP_AE_ERR_OK && output_samples > 0 &&
                    output_samples <= _input_resampler_max_samples) {
                resample_failures = 0;
                _audio_processor->feed(chat_pcm, output_samples);
            } else {
                ++resample_failures;
...
void XiaozhiApp::playAudio()
{
    ...
    uint32_t playback_packets_written = 0;
    AudioPacket packet = {};
    while (server_pcm && device_pcm && _audio_tasks_running.load()) {
        bool received = false;
        bool decoded = false;
        bool output_error = false;
        size_t playback_samples = 0;
        int16_t *playback_pcm = nullptr;
        if (_playback_queue) {
            size_t item_size = 0;
            void *ring_item = xRingbufferReceive(
                _playback_queue, &item_size, pdMS_TO_TICKS(50)
            );
            if (ring_item) {
                if (item_size == sizeof(packet)) {
                    memcpy(&packet, ring_item, sizeof(packet));
                    received = true;
                    _playback_busy.store(true);
                } else {
                    ESP_UTILS_LOGW(
                        "Drop invalid Xiaozhi playback item: %u bytes",
                        static_cast<unsigned int>(item_size)
                    );
                }
                vRingbufferReturnItem(_playback_queue, ring_item);
            }
        }
        if (!received) {
                    ...
                    if (_output_resampler) {
                        ...
                        if (esp_ae_rate_cvt_get_max_out_sample_num(
                            ...
                            if (esp_ae_rate_cvt_process(
                                    ...
                                    actual_samples > 0 &&
                                    actual_samples <= MAX_DEVICE_PCM_SAMPLES) {
        ...
        if (output_error) {
            ...
            if (decode_failures == 1 || (decode_failures % 20) == 0) {
                ...
                packet.turn_generation == _turn_generation.load()) {
            size_t written = 0;
            size_t write_size = playback_samples * sizeof(int16_t);
            esp_err_t write_ret = bsp_extra_xiaozhi_audio_write(
                playback_pcm, write_size, &written, 200
            );
            if (write_ret == ESP_OK && written == write_size) {
                if (playback_packets_written++ == 0) {
                    ESP_UTILS_LOGI("Xiaozhi audio playback started");
                }
...
void XiaozhiApp::uiTimerCallback(lv_timer_t *timer)
{
    ...
    if (instance) {
        if (instance->isNetworkReady() != instance->_network_ready.load()) {
